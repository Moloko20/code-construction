...
export function UsersList() {
    const users = useAppSelector(state => state.usersReducer.filteredUsers)
    ...
    const search = (event: React.ChangeEvent<HTMLInputElement>) => {
        const filteredPosts = defaultUsers.filter(user => {
            const userFiltered = [
                user.id,
                user.name,
                user.company?.name,
            ...
            return userFiltered.toString().includes(event.target.value)
        })

        dispatch({ type: UsersActionTypes.SET_USERS, payload: filteredPosts })
    }

    React.useEffect(() => {
        dispatch({ type: UsersActionTypes.SET_USERS, payload: defaultUsers })
    }, [])
        ...
        <section className="userslist">
            <div className="page__layout">
                <div className="userslist-header">
                    <div className="userslist-header__left">
                        <svg
                            width="25"
                            height="25"
                            viewBox="0 0 20 25"
